...
export default function Features() {
  ...
  return (
    <section ref={container} className="relative py-32 bg-background overflow-hidden">
      {/* Background Grid */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="mb-20">
            <h2 className="text-4xl md:text-6xl font-black text-foreground mb-6 uppercase tracking-tighter">
             ...
             <div className="w-full h-px bg-muted mt-8"></div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {skills.map((skill, idx) => (
            <TiltCard key={idx} className="feature-card">
              <div className="p-8 rounded-lg border border-border bg-gradient-to-br from-white/5 to-white/[0.02] hover:border-border transition-all duration-300">
                <h3 className="text-2xl font-bold text-foreground mb-6">{skill.category}</h3>
                <div className="flex flex-wrap gap-3">
                  {skill.items.map((item, idx) => (
                    <span key={idx} className="px-3 py-1 rounded-full bg-muted text-foreground/80 text-sm font-medium">
                      {item}
                    </span>
                  ))}
                </div>
