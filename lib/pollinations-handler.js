  ...
  return {
    ...
    async generateImage({ prompt, model = 'flux', width = 1024, height = 1024 }) {
      try {
        ...
        const url = `https://gen.pollinations.ai/image/${encodedPrompt}?model=${model}&width=${width}&height=${height}`
    ...
    async generateVideo({ imageUrl, prompt, model = 'grok-video', duration = 5 }) {
      try {
        const response = await fetch('/api/videos/generate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ imageUrl, prompt, model, duration }),
        })
