...
async function connectDB() {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose
      ...
      .catch(async (error) => {
        const errorString = String(error) + (error.reason ? String(error.reason) : '');
        const isSrvDnsIssue =
          (errorString.includes('ECONNREFUSED') || errorString.includes('querySrv') || errorString.includes('MongooseServerSelectionError')) &&
